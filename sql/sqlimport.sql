...
(1, '<p>Vi kan ta över punkten genom att dansa conga!</p>\n', 'admin', 1, '2015-11-23 17:47:42', 5, 'https://s.gravatar.com/avatar/5ce10f6be81a5a7227fe5b03cd8f8d44?s=80'),
(2, '<p>Låter som en bra idé!</p>\n', 'Spion', 1, '2015-11-23 17:48:07', 15, 'https://s.gravatar.com/avatar/ca7b5e615e7f2f5e57da63851131d667?s=80'),
(3, '<p>jag med</p>\n', 'doe', 3, '2015-11-23 20:25:57', 3, 'https://s.gravatar.com/avatar/26b712c3864f409f6d456ba2e053e59d?s=80');
...
(1, 1, 'Test', '<p>Det kan vi prova!</p>\n', '2015-11-23 16:48:51', 17, 'https://secure.gravatar.com/userimage/57683588/b57b88b806ee7cb226b606304d652387.png');
...
(1, '<p>Hur ska vi ta över punkten?</p>\n', 'doe', NULL, NULL, NULL, '2015-11-23 17:46:07', '194.47.129.122', 'https://s.gravatar.com/avatar/26b712c3864f409f6d456ba2e053e59d?s=80', 3, 'questions'),
(2, '<p>Är det någon som vill spela heavy?</p>\n', 'Spion', NULL, NULL, NULL, '2015-11-23 17:50:13', '194.47.129.126', 'https://s.gravatar.com/avatar/ca7b5e615e7f2f5e57da63851131d667?s=80', 15, 'questions'),
(3, '<p>Lagmedlemmar sökes!</p>\n', 'doe', NULL, NULL, NULL, '2015-11-23 17:50:44', '194.47.129.122', 'https://s.gravatar.com/avatar/26b712c3864f409f6d456ba2e053e59d?s=80', 3, 'questions');
...
(3, 'doe', 'doe@mail.com', 'doe', 'https://s.gravatar.com/avatar/26b712c3864f409f6d456ba2e053e59d?s=80', '$2y$10$GNzMlmUAn9KFTSSV2Erc4uBbyRddWjGqdyGvarm14cX.HdB0n8Yma', '2015-11-14 15:56:37', NULL, NULL, '2015-11-14 15:56:37'),
(5, 'admin', 'admin@admin.com', 'admin', 'https://s.gravatar.com/avatar/5ce10f6be81a5a7227fe5b03cd8f8d44?s=80', '$2y$10$ubD2eE1MVWWKUNoGRoGrHu6KijZu5e.wo4uo5nFL1/3Ir/MvG0b4m', '2015-11-14 15:58:00', NULL, NULL, '2015-11-14 15:58:00'),
(15, 'Spion', 'Spion@email.com', 'Spion', 'https://s.gravatar.com/avatar/ca7b5e615e7f2f5e57da63851131d667?s=80', '$2y$10$.jfK8emJCvmAhLsLPQ5jYOibxbZDIvVzBKVBwWMNaf93NKgZTSaLS', '2015-11-16 14:22:56', NULL, NULL, '2015-11-16 14:22:56'),
(17, 'Test', 'test@test.coma', 'Hej', 'https://secure.gravatar.com/userimage/57683588/b57b88b806ee7cb226b606304d652387.png', '$2y$10$9QadCEGykLk7m/UjGF0wseSF1kd.JqcQKi10zDksXAZl2vNsa.fFS', '2015-11-23 15:38:00', NULL, NULL, '2015-11-23 15:38:00');
...
CREATE ALGORITHM=UNDEFINED DEFINER=`milc15`@`%` SQL SECURITY DEFINER VIEW `phpmvc_commentsview` AS select `c`.`question_id` AS `question_id`,`c`.`comments` AS `comments`,`c`.`author` AS `author`,`c`.`timestamp` AS `timestamp`,`q`.`userid` AS `userid`,`q`.`id` AS `id` from (`phpmvc_comments` `c` left join `phpmvc_question` `q` on((`c`.`question_id` = `q`.`id`)));
...
CREATE ALGORITHM=UNDEFINED DEFINER=`milc15`@`%` SQL SECURITY DEFINER VIEW `phpmvc_tagsview` AS select `T`.`tags` AS `tags`,`QT`.`questionId` AS `questionId` from ((`phpmvc_tags` `T` join `phpmvc_questiontags` `QT` on((`QT`.`tags_id` = `T`.`id`))) join `phpmvc_question` `Q` on((`Q`.`id` = `QT`.`questionId`)));
